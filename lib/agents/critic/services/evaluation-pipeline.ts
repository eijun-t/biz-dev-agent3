...
import { SynergyScoringService } from './synergy-scoring-service';
import { LLMEvaluator } from './llm-evaluator';
import { withTimeout, PartialResultError } from '../errors';
import { v4 as uuidv4 } from 'uuid';

/**
 * Evaluation Pipeline Configuration
 */
export interface EvaluationPipelineConfig extends CriticConfig {
...
export class EvaluationPipeline {
  private marketScorer: MarketScoringService;
  private synergyScorer: SynergyScoringService;
  private llmEvaluator: LLMEvaluator;
  private config: Required<EvaluationPipelineConfig>;
  ...
  constructor(config?: EvaluationPipelineConfig) {
    ...
    // サービスの初期化
    this.marketScorer = new MarketScoringService({
      llmModel: this.config.llmModel,
      temperature: this.config.temperature,
    });

    this.synergyScorer = new SynergyScoringService({
      llmModel: this.config.llmModel,
      temperature: this.config.temperature,
    });
  ...
  async evaluate(input: CriticInput): Promise<CriticOutput> {
    ...
    try {
      ...
      if (error instanceof PartialResultError) {
        ...
        // 部分的な結果から最良のものを選択
        const partialResults = error.partialResults as EvaluationResult[];
   ...
   * 複数アイデアの評価（並列処理）
   */
  private async evaluateIdeas(ideas: BusinessIdea[]): Promise<EvaluationResult[]> {
    if (this.config.parallelProcessing) {
      ...
      for (const chunk of chunks) {
        const chunkResults = await Promise.all(
          chunk.map(idea => this.evaluateSingleIdea(idea))
        );
   ...
   * 単一アイデアの評価
   */
  private async evaluateSingleIdea(idea: BusinessIdea): Promise<EvaluationResult> {
    // 30秒のタイムアウト設定
    return withTimeout(
      this.evaluateIdeaInternal(idea),
      30000,
      `Evaluation timeout for idea: ${idea.title}`
    );
  }
   ...
   * アイデア評価の内部実装
   */
  private async evaluateIdeaInternal(idea: BusinessIdea): Promise<EvaluationResult> {
    // 市場評価とシナジー評価を並列実行
   ...
   * 合計スコアの計算
   */
  private calculateTotalScore(marketScore: number, synergyScore: number): number {
   ...
   * 最優秀アイデアの選定
   */
  selectBestIdea(results: EvaluationResult[]): EvaluationResult {
    if (results.length === 0) {
    ...
    if (sorted[0].totalScore < this.config.minimumTotalScore) {
      console.warn(
        `Best idea score (${sorted[0].totalScore}) is below minimum threshold (${this.config.minimumTotalScore})`
      );
    }
   ...
   * 結果のランキング付け
   */
  private rankResults(results: EvaluationResult[]): EvaluationResult[] {
    const sorted = [...results].sort((a, b) => b.totalScore - a.totalScore);
