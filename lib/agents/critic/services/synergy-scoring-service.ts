...
/**
 * Synergy Scoring Service Configuration
 */
export interface SynergyScoringConfig {
  llmModel?: string;
  temperature?: number;
  enableMultiStageEvaluation?: boolean;
}

/**
 * Synergy Scoring Service
 */
export class SynergyScoringService {
  private llmEvaluator: LLMEvaluator;
  private config: Required<SynergyScoringConfig>;

  constructor(config?: SynergyScoringConfig) {
    this.config = {
   ...
   * Stage 1: ケイパビリティマッピング
   */
  private async evaluateCapabilityMapping(idea: BusinessIdea): Promise<CapabilityMapping> {
    const mitsubishiCapabilities = generateCapabilityDescription();
   ...
   * Stage 2: シナジーシナリオ生成
   */
  private async generateSynergyScenario(
    idea: BusinessIdea,
    capabilityMapping: CapabilityMapping
  ): Promise<SynergyScenario> {
    const llmResult = await this.llmEvaluator.generateSynergyScenario(
      idea,
      capabilityMapping.mitsubishiCapabilities
    );
   ...
   * Stage 3: シナリオ検証
   */
  private async validateScenario(synergyScenario: SynergyScenario): Promise<ScenarioValidation> {
    const llmResult = await this.llmEvaluator.validateScenario(
      synergyScenario.scenario,
      synergyScenario.keyAdvantages
    );
  ...
  private calculateSynergyScore(
    ...
    const synergyEffect = Math.round(
      ((synergyScenario.synergyMultiplier - 1.0) / 0.5) * 10 + // 乗数効果（最大10点）
      (scenarioValidation.feasibility / 100) * 5 // 実現可能性（最大5点）
    );

    // 独自優位性 (0-15点)
    // 独自性と総合的な納得度から計算
    const uniqueAdvantage = Math.round(
      (scenarioValidation.uniqueness / 100) * 10 + // 独自性（最大10点）
  ...
  async generateDetailedReport(
    ...
    const capabilities = getFlatCapabilityList();
